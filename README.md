...
* MLnet
使用ML.net功能，vs2019开始提供了机器学习的功能，通过界面引导方式，进行配置相关模型，10分钟就可以完成一个简单的分类问题的机器学习过程。相比其他语言中的框架模型，MLnet更重视产品的使用体验，以及快速为真实场景服务，而不是模型的研究调优等。
...
** MLLearnML.Model 通过引导界面生成的dll项目，根据文本输入和文本代表的是积极还是消极，进行训练。

** MLLearnML.ConsoleApp 直接引用MLLearnML.Model的一个测试程序，调用相关方法进行预测。

** MLPredictiveMaintenanceML.Model 通过引导界面生成的dll项目，用途是针对传感器的数据，以及传感器当时的好坏情况，进行训练。
